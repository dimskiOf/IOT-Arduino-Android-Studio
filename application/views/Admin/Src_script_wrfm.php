...
function onMessageArrived(message) {
 ...
 // table.ajax.reload();
  }else if(message.payloadString == 'data rmmpi update'){
   tabled.ajax.reload(); 
  }else if(message.payloadString == 'data rmmpi masuk'){
    tabless.ajax.reload();
  }else if(message.payloadString == 'data rmmpi keluar'){
    ...
    $(document).ready( function () {
       ...
       "processing": true,
        ...
        "order": [[ 0, 'desc' ]],
        "ajax": {
            "url": "<?php echo base_url('mpi/loadrm')?>",
            "type": "POST"
        },
       dom : 'Bfrtip',
        "buttons": [{
...
//get data rm masuk
    $(document).ready( function () {
       ...
       "processing": true,
        ...
        "order": [[ 0, 'desc' ]],
        "ajax": {
            "url": "<?php echo base_url('mpi/loadrmmasuk')?>",
            "type": "POST",
    ...
    $(document).ready( function () {
       ...
       "processing": true,
        ...
        "order": [[ 0, 'desc' ]],
        "ajax": {
            "url": "<?php echo base_url('mpi/loadrmkeluar')?>",
            "type": "POST",
            "data" : function(d){ d.datefilter = $('#rmmasukfilter').data('daterangepicker').startDate.format('YYYY-MM-DD')+"#start#"+$('#rmmasukfilter').data('daterangepicker').endDate.format('YYYY-MM-DD')+"#start#" }
        },
        "columnDefs": [
...
$(document).ready( function () {
       ...
       "processing": true,
        ...
        "order": [[ 0, 'asc' ]],
        "ajax": {
            "url": "<?php echo base_url('mpi/getadddatarm')?>",
            "type": "POST"
        },
        "columnDefs": [
            {
...
function hapusrmkeluar(idi)
    {
        ...
        $.ajax({
                  type: "POST",
                  url: "<?php echo base_url(); ?>mpi/hapusrmkeluar",
                  data: {id : idi},
                  dataType:"json",
 ...
 function hapusrmmasuk(idi)
    {
        ...
        $.ajax({
                  type: "POST",
                  url: "<?php echo base_url(); ?>mpi/hapusrmmasuk",
                  data: {id : idi},
                  dataType:"json",
                  success: function (data) {
                    $.each(data,function(pesan){
...
function hapusadditem(idi)
    {
        ...
        $.ajax({
                  type: "POST",
                  url: "<?php echo base_url(); ?>mpi/hapusadditem",
                  data: {id : idi},
                  dataType:"json",
                  success: function (data) {
                    $.each(data,function(pesan){
...
$(document).ready(function(){
 ...
 $("#selUser").select2({
  ajax: { 
   url: "<?php echo base_url('mpi/select2get') ?>",
   type: "post",
   dataType: 'json',
   delay: 250,
   ...
   cache: true
    ...
    $(document).ready(function () {
        ...
        $("#printthisbarcode").click(function (event){
            window.open('<?php echo base_url('mpi/printbarcoderm'); ?>','_blank');
         })
          ...
          $("#senddata").click(function (event) {
              ...
              $.ajax({
                  ...
                  url: "<?php echo base_url('mpi/addbarcode'); ?>",
                  data: data,
                  processData: false,
                  contentType: false,
                  cache: false,
