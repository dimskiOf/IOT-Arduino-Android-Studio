...
function onMessageArrived(message) {
 ...
 // table.ajax.reload();
  }else if(message.payloadString == 'data fgmpi keluar'){
    tabless.ajax.reload();
  }
}
  ...
  $('#fgkeluarfilter').daterangepicker({
    ...
    $(document).ready( function () {
       window.tabless = $('#fgkeluar').DataTable({ 
        "processing": true,
        "serverSide": true,
        "order": [],
        "ajax": {
            "url": "<?php echo base_url('fgkeluar/loadfgkeluar')?>",
            "type": "POST",
        ...
        "columnDefs": [
            ...
            { 
                ...
                "mRender" : function(data, type, full){
                    return '<tr><td><button class="btn btn-sm btn-danger text-white" onclick=hapusfgkeluar("'+full[9]+'"); title="Hapus Item"><i class="fa fa-trash"></i></button></td></tr>';
                }
            }],
       dom : 'Bfrtip',
        "buttons": [{
...
$(document).ready( function () {
       ...
       "processing": true,
        ...
        "order": [],
        "ajax": {
            "url": "<?php echo base_url('fgkeluar/getadddatafg')?>",
            "type": "POST"
        },
        "columnDefs": [
            {
...
function hapusfgkeluar(idi)
    {
        ...
        $.ajax({
                  type: "POST",
                  url: "<?php echo base_url(); ?>fgkeluar/hapusfgkeluar",
                  data: {id : idi},
                  dataType:"json",
    ...
    function hapusadditem(idi)
    {
        ...
        $.ajax({
                  type: "POST",
                  url: "<?php echo base_url(); ?>fgkeluar/hapusadditemfg",
                  data: {id : idi},
                  dataType:"json",
                  success: function (data) {
                    $.each(data,function(pesan){
...
$(document).ready(function(){
 ...
 $("#selUser").select2({
  ajax: { 
   url: "<?php echo base_url('fgkeluar/select2getfg') ?>",
   type: "post",
   dataType: 'json',
   delay: 250,
   ...
   cache: true
    ...
    $(document).ready(function () {
        ...
        $("#printthisbarcode").click(function (event){
            window.open('<?php echo base_url('fgkeluar/printbarcodefg'); ?>','_blank');
         })
          ...
          $("#senddata").click(function (event) {
              ...
              $.ajax({
                  ...
                  url: "<?php echo base_url('fgkeluar/addbarcodefg'); ?>",
                  data: data,
                  processData: false,
                  contentType: false,
                  cache: false,
