<?php
class Get_rmmpi_keluarmasuk extends CI_Model {
   ...
   function get_rmmpi_keluar($query,$cari,$where,$iswhere)
        {
        ...
        function get_rmmpi_masuk($query,$cari,$where,$iswhere)
        {
            // Ambil data yang di ketik user pada textbox pencarian
            $search = htmlspecialchars($_POST['search']['value']);
            // Ambil data limit per page
       ...
       {
        $this->db->where('ID_RM_KELUAR', $this->input->post('id'));
        return  $this->db->delete('WAREHOUSE_RM_KELUAR');
       }
