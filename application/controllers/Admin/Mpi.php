...
class Mpi extends CI_Controller {
    ...
    function __construct(){
        ...
        switch ($this->session->userdata('privilages')) {
          ...
          $this->load->model('Get_raw_mpi');
          $this->load->model('Get_add_rm_mpi');
          $this->load->model('In_raw_mpi');
          $this->load->model('Get_rmmpi_keluarmasuk');
    }
 

    public function index()
    {
      $this->load->view('Admin/header_warehouserm');
      $this->load->view('Admin/Modal_warehouserm');
      $this->load->view('Admin/navbar');
      $this->load->view('Admin/sidebar');
      $this->load->view('Admin/konten_warehouserm');
      $this->load->view('Admin/footer');
      $this->load->view('Admin/src_script_wrfm');
      $this->load->view('Admin/static_script');
      $this->load->view('Admin/end_of_tag');
    }

    //INI RAW MATERIALNYA MPI
    public function getdatarm(){
         ...
         $query  = "
            ...
            echo json_encode($this->Get_raw_mpi->get_tables_query($query,$cari,$where,$isWhere));
    }
    ...
    public function getadddatarm()
    {
       $datenow = date('m/d/Y');
         $query  = 'rm.ID_ADD_RM, rm.URL,rm."FILE", rm.JML, r.ITEMNO, r.ITEMDESCRIPTION, r.ITEMTYPE, r.SUBITEM, r.PARENTITEM,
            ...
            echo json_encode($this->Get_add_rm_mpi->get_tables_query($query,$cari,$where,$isWhere));
    }
    ...
    public function select2getdata()
    {
         ...
         $query  = "
            ...
            if(!isset($_POST['searchTerm'])){ 
              $fetchData = $this->Get_raw_mpi->getbyalldataraw('select first 10 '.$query);
            }else{ 
              $search = $_POST['searchTerm'];   
              $fetchData = $this->Get_raw_mpi->getbyalldataraw('select first 10 '.$query.'where a.ITEMNO like '."'%".$search."%'");
            } 
    ...
    public function insertdataforprinting()
    {
        ...
        $nomor = $this->Get_raw_mpi->get_numbers(); 

        foreach($data AS $key => $val){

        $stat = $this->In_raw_mpi->inputdata($_POST['addbarcoder'][$key],'null',$nomor++,$this->input->post('addjumlah'));
      ...
      if (!empty($data))
      {
        $status = $this->db->get_where('ADD_RM', array('ID_ADD_RM' => $this->input->post('id')));
         if ($status->num_rows() > 0){ 
          $this->Get_add_rm_mpi->hapusadditem();
    ...
    public function getdatarmkeluar()
    {
      ...
      echo json_encode($this->Get_rmmpi_keluarmasuk->get_rmmpi_keluar($query,$cari,$where,$isWhere));
    }
    ...
    public function getdatarmmasuk()
    {
      ...
      echo json_encode($this->Get_rmmpi_keluarmasuk->get_rmmpi_masuk($query,$cari,$where,$isWhere));
    }
    ...
    public function hapusdatarmkeluar(){
      ...
      if (!empty($data))
      {
        $status = $this->db->get_where('WAREHOUSE_RM_KELUAR', array('ID_RM_KELUAR' => $this->input->post('id')));
         if ($status->num_rows() > 0){ 
          $this->Get_rmmpi_keluarmasuk->hapusrmkeluar();
          echo json_encode(array('pesan' => "<div class='alert alert-success'>Hapus Data Berhasil</div>"));
         }else{
           echo json_encode(array('pesan' => "<div class='alert alert-danger'>DATA TIDAK ADA</div>"));
         }
    ...
    public function hapusdatarmmasuk(){
      ...
      if (!empty($data))
      {
        $status = $this->db->get_where('WAREHOUSE_RM_MASUK', array('ID_RM_MASUK' => $this->input->post('id')));
         if ($status->num_rows() > 0){ 
          $this->Get_rmmpi_keluarmasuk->hapusrmmasuk();
          echo json_encode(array('pesan' => "<div class='alert alert-success'>Hapus Data Berhasil</div>"));
         }else{
           echo json_encode(array('pesan' => "<div class='alert alert-danger'>DATA TIDAK ADA</div>"));
         }
