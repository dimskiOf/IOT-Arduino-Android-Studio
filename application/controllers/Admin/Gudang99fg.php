...
class Gudang99fg extends CI_Controller {
    ...
    function __construct(){
        ...
        switch ($this->session->userdata('privilages')) {
          ...
          $this->load->model('Get_fg_gd99');
          $this->load->model('Get_add_fg_gd99');
          $this->load->model('In_fg_gd99');
          $this->load->model('Get_fg99_keluarmasuk');
    }
 

    public function index()
    {
      $this->load->view('Admin99/header_warehousefg');
      $this->load->view('Admin99/Modal_warehousefg');
      $this->load->view('Admin99/navbar');
      $this->load->view('Admin99/sidebar');
      $this->load->view('Admin99/konten_warehousefg');
      $this->load->view('Admin99/footer');
      $this->load->view('Admin99/src_script_wrfg');
      $this->load->view('Admin99/static_script');
      $this->load->view('Admin99/end_of_tag');
    }

        //INI FG MATERIALNYA GUDANG 99
    public function getdatafg(){
         ...
         $query  = "
            ...
            echo json_encode($this->Get_fg_gd99->get_tables_query($query,$cari,$where,$isWhere));
    }
    ...
    public function getadddata()
    {
       $datenow = date('m/d/Y');
         $query  = 'fg.ID_ADD_FG, fg.URL,fg."FILE", fg.JML, r.ITEMNO, r.ITEMDESCRIPTION, r.ITEMTYPE, r.SUBITEM, r.PARENTITEM,
            ...
            echo json_encode($this->Get_add_fg_gd99->get_tables_query($query,$cari,$where,$isWhere));
    }
    ...
    public function select2getdata()
    {
         ...
         $query  = "
            ...
            if(!isset($_POST['searchTerm'])){ 
              $fetchData = $this->Get_fg_gd99->getbyalldataraw('select first 10 '.$query);
            }else{ 
              $search = $_POST['searchTerm'];   
              $fetchData = $this->Get_fg_gd99->getbyalldataraw('select first 10 '.$query.'where a.ITEMNO like '."'%".$search."%'");
            } 
    ...
    public function insertdataforprinting()
    {
        ...
        $nomor = $this->Get_fg_gd99->get_numbers(); 

        foreach($data AS $key => $val){

        $stat = $this->In_fg_gd99->inputdata($_POST['addbarcoder'][$key],'null',$nomor++,$this->input->post('addjumlah'));
    ...
    public function hapusadditem()
    {
      ...
      if (!empty($data))
      {
        ...
        $status = $gd99->get_where('ADD_FG', array('ID_ADD_FG' => $this->input->post('id')));
         if ($status->num_rows() > 0){ 
          $this->Get_add_fg_gd99->hapusadditem();
    ...
    public function getdatafgkeluar()
    {
      ...
      echo json_encode($this->Get_fg99_keluarmasuk->get_fg99_keluar($query,$cari,$where,$isWhere));
    }
    ...
    public function getdatafgmasuk()
    {
      ...
      echo json_encode($this->Get_fg99_keluarmasuk->get_fg99_masuk($query,$cari,$where,$isWhere));
    }
    ...
    public function hapusdatafgkeluar(){
      ...
      {
        $gd99 = $this->load->database('gd99', TRUE);
        $status = $gd99->get_where('WAREHOUSE_FG_KELUAR', array('ID_FG_KELUAR' => $this->input->post('id')));
         if ($status->num_rows() > 0){ 
          $this->Get_fg99_keluarmasuk->hapusfgkeluar();
          echo json_encode(array('pesan' => "<div class='alert alert-success'>Hapus Data Berhasil</div>"));
         }else{
           echo json_encode(array('pesan' => "<div class='alert alert-danger'>DATA TIDAK ADA</div>"));
         }
  ...
  //hapus data fg masuk
    public function hapusdatafgmasuk(){
      ...
      {
        $gd99 = $this->load->database('gd99', TRUE);
        $status = $gd99->get_where('WAREHOUSE_FG_MASUK', array('ID_FG_MASUK' => $this->input->post('id')));
         if ($status->num_rows() > 0){ 
          $this->Get_fg99_keluarmasuk->hapusfgmasuk();
          echo json_encode(array('pesan' => "<div class='alert alert-success'>Hapus Data Berhasil</div>"));
         }else{
           echo json_encode(array('pesan' => "<div class='alert alert-danger'>DATA TIDAK ADA</div>"));
         }
