...
class Rmkeluar_c extends CI_Controller {

    function __construct(){
        parent::__construct();
        $this->load->model('Get_rm_keluar');
    ...
    {   
        ...
        $list = $this->Get_rm_keluar->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->Get_rm_keluar->count_all(),
            "recordsFiltered" => $this->Get_rm_keluar->count_filtered(),
            "data" => $data,
        );
          ...
          if (!empty($data))
          {
            $status = $this->db->get_where('warehouse_rm_keluar', array('id_rm_keluar' => $this->input->post('id')));
             if ($status->num_rows() > 0){ 
              $this->Get_rm_keluar->hapusrmkeluar();
     ...
     public function getadddata()
        {
        $list = $this->Get_rm_keluar->get_datatables_barcode();
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $field) {
          $row[] = $no;
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->Get_rm_keluar->count_all_barcode(),
            "recordsFiltered" => $this->Get_rm_keluar->count_filtered_barcode(),
            "data" => $data,
        );
        //output dalam format JSON
        echo json_encode($output);
        ...
        public function select2getdata(){
           if(!isset($_POST['searchTerm'])){ 
              $fetchData = $this->Get_rm_keluar->getbyalldataraw('select * from item limit 10');
            }else{ 
              $search = $_POST['searchTerm'];   
              $fetchData = $this->Get_rm_keluar->getbyalldataraw('select * from item where itemno like '."'%".$search."%'");
            } 
        ...
        public function insertdataforprinting()
        {
            ...
            $nomor = $this->Get_rm_keluar->get_numbers(); 

            foreach($data AS $key => $val){

            $stat = $this->Get_rm_keluar->inputdata($_POST['addbarcoder'][$key],'null',$nomor++,$this->input->post('addjumlah'));
        ...
        public function hapusadditem()
        {
          ...
          if (!empty($data))
          {
            $status = $this->db->get_where('add_rm', array('id_add_rm' => $this->input->post('id')));
             if ($status->num_rows() > 0){ 
              $this->Get_rm_keluar->hapusadditem();
              echo json_encode(array('pesan' => "<div class='alert alert-success'>Hapus Data Berhasil</div>"));
             }else{
               echo json_encode(array('pesan' => "<div class='alert alert-danger'>DATA TIDAK ADA</div>"));
             }
